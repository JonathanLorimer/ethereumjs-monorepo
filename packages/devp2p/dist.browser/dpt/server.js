...
class Server extends events_1.EventEmitter {
    constructor(dpt, privateKey, options) {
        ...
        this._requestsCache = new LRUCache({ max: 1000, maxAge: (0, ms_1.default)('1s'), stale: false });
    ...
    async ping(peer) {
        ...
        const deferred = (0, util_1.createDeferred)();
        ...
        this._requests.set(rkey, {
            ...
            timeoutId: setTimeout(() => {
                if (this._requests.get(rkey) !== undefined) {
                    this._debug(`ping timeout: ${peer.address}:${peer.udpPort} ${peer.id ? (0, util_1.formatLogId)(peer.id.toString('hex'), verbose) : '-'}`);
    ...
    _send(peer, typename, data) {
        const debugMsg = `send ${typename} to ${peer.address}:${peer.udpPort} (peerId: ${peer.id ? (0, util_1.formatLogId)(peer.id.toString('hex'), verbose) : '-'})`;
        this.debug(typename, debugMsg);
        const msg = (0, message_1.encode)(typename, data, this._privateKey);
        // Parity hack
        ...
        // 2018-02-28
        if (typename === 'ping') {
            const rkeyParity = (0, util_1.keccak256)(msg.slice(98)).toString('hex');
    ...
    _handler(msg, rinfo) {
        const info = (0, message_1.decode)(msg);
        const peerId = (0, util_1.pk2id)(info.publicKey);
        const debugMsg = `received ${info.typename} from ${rinfo.address}:${rinfo.port} (peerId: ${(0, util_1.formatLogId)(peerId.toString('hex'), verbose)})`;
        ...
        if (peer === null && info.typename === 'ping' && info.data.from.udpPort !== null) {
            setTimeout(() => this.emit('peers', [info.data.from]), (0, ms_1.default)('100ms'));
        }
