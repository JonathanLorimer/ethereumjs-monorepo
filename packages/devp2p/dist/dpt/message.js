...
const debug = (0, debug_1.debug)('devp2p:dpt:server');
...
const port = {
    ...
    decode: function (buffer) {
        ...
        return (0, util_2.buffer2int)(buffer);
    },
};
const endpoint = {
    encode: function (obj) {
...
const ping = {
    encode: function (obj) {
        return [
            (0, util_2.int2buffer)(obj.version),
    ...
    decode: function (payload) {
        return {
            version: (0, util_2.buffer2int)(payload[0]),
...
function encode(typename, data, privateKey) {
    ...
    const typedata = Buffer.concat([
        Buffer.from([type]),
        Buffer.from(rlp_1.default.encode((0, util_1.bufArrToArr)(encodedMsg))),
    ]);
    const sighash = (0, util_2.keccak256)(typedata);
    ...
    const hash = (0, util_2.keccak256)(hashdata);
    return Buffer.concat([hash, hashdata]);
}
exports.encode = encode;
function decode(buffer) {
    const hash = (0, util_2.keccak256)(buffer.slice(32));
    (0, util_2.assertEq)(buffer.slice(0, 32), hash, 'Hash verification failed', debug);
    const typedata = buffer.slice(97);
    const type = typedata[0];
    const typename = types.byType[type];
    if (typename === undefined)
        throw new Error(`Invalid type: ${type}`);
    const data = messages[typename].decode((0, util_2.unstrictDecode)(typedata.slice(1)));
    const sighash = (0, util_2.keccak256)(typedata);
