...
class DefaultStateManager extends _1.BaseStateManager {
    ...
    async getContractStorage(address, key) {
        ...
        const decoded = Buffer.from(rlp_1.default.decode(Uint8Array.from(value ?? [])));
        return decoded;
    }
    /**
    ...
    async putContractStorage(address, key, value) {
        ...
        await this._modifyContractStorage(address, async (storageTrie, done) => {
            if (value && value.length) {
                // format input
                const encodedValue = Buffer.from(rlp_1.default.encode(Uint8Array.from(value)));
                if (this.DEBUG) {
    ...
    async getProof(address, storageSlots = []) {
        const account = await this.getAccount(address);
        const accountProof = (await trie_1.SecureTrie.createProof(this._trie, address.buf)).map((p) => (0, util_1.bufferToHex)(p));
        const storageProof = [];
        const storageTrie = await this._getStorageTrie(address);
        for (const storageKey of storageSlots) {
            const proof = (await trie_1.SecureTrie.createProof(storageTrie, storageKey)).map((p) => (0, util_1.bufferToHex)(p));
        ...
        const returnValue = {
            ...
            storageHash: (0, util_1.bufferToHex)(account.stateRoot),
            accountProof,
            storageProof,
        };
        return returnValue;
    ...
    async verifyProof(proof) {
        ...
        const value = await trie_1.SecureTrie.verifyProof(rootHash, key, accountProof);
        if (value === null) {
            ...
            if (!stateRoot.equals((0, util_1.toBuffer)(proof.storageHash))) {
        ...
        for (const stProof of proof.storageProof) {
            ...
            const proofValue = await trie_1.SecureTrie.verifyProof(storageRoot, storageKey, storageProof);
            const reportedValue = (0, util_1.setLengthLeft)(Buffer.from(rlp_1.default.decode(Uint8Array.from(proofValue ?? []))), 32);
