...
class Trie {
     ...
     */
    constructor(opts) {
        this.EMPTY_TRIE_ROOT = util_1.KECCAK256_RLP;
        this.lock = new semaphore_async_await_1.default(1);
        this.db = opts?.db ?? new db_1.LevelDB();
    ...
    set root(value) {
        ...
        if (value.length !== 32)
            throw new Error('Invalid root length. Roots are 32 bytes');
        this._root = value;
    }
    /**
    ...
    async put(key, value) {
        ...
        if (this.root.equals(util_1.KECCAK256_RLP)) {
    ...
    _formatNode(node, topLevel, opStack, remove = false) {
        ...
        if (rlpNode.length >= 32 || topLevel) {
            ...
            const hashRoot = Buffer.from((0, keccak_1.keccak256)(rlpNode));
            if (remove) {
    ...
    static async fromProof(proof, trie) {
        const opStack = proof.map((nodeValue) => {
            return {
                type: 'put',
                key: Buffer.from((0, keccak_1.keccak256)(nodeValue)),
                value: nodeValue,
            };
        });
        if (!trie) {
