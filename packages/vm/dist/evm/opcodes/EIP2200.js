...
function updateSstoreGasEIP2200(runState, currentStorage, originalStorage, value, key, common) {
    ...
    if (originalStorage.equals(currentStorage)) {
        ...
        // Delete slot
        if (value.length === 0) {
            runState.eei.refundGas(common.param('gasPrices', 'sstoreClearRefundEIP2200'), 'EIP-2200 -> sstoreClearRefundEIP2200');
        }
        // Write existing slot
    ...
    if (originalStorage.length > 0) {
        if (currentStorage.length === 0) {
            // Recreate slot
            runState.eei.subRefund(common.param('gasPrices', 'sstoreClearRefundEIP2200'), 'EIP-2200 -> sstoreClearRefundEIP2200');
        }
    ...
    if (originalStorage.equals(value)) {
        if (originalStorage.length === 0) {
            ...
            runState.eei.refundGas(BigInt((0, EIP2929_1.adjustSstoreGasEIP2929)(runState, key, sstoreCleanRefund, 'cleanRefund', common)), 'EIP-2200 -> cleanRefund');
        }
    }
    // Dirty update
