...
function updateSstoreGasEIP1283(runState, currentStorage, originalStorage, value, common) {
    ...
    if (originalStorage.equals(currentStorage)) {
        ...
        if (value.length === 0) {
            ...
            runState.eei.refundGas(common.param('gasPrices', 'netSstoreClearRefund'), 'EIP-1283 -> netSstoreClearRefund');
        }
    ...
    if (originalStorage.length !== 0) {
        ...
        if (currentStorage.length === 0) {
            ...
            runState.eei.subRefund(common.param('gasPrices', 'netSstoreClearRefund'), 'EIP-1283 -> netSstoreClearRefund');
        }
    ...
    if (originalStorage.equals(value)) {
        ...
        if (originalStorage.length === 0) {
            ...
            runState.eei.refundGas(common.param('gasPrices', 'netSstoreResetClearRefund'), 'EIP-1283 -> netSstoreResetClearRefund');
        }
        else {
            // Otherwise, add 4800 gas to refund counter.
            runState.eei.refundGas(common.param('gasPrices', 'netSstoreResetRefund'), 'EIP-1283 -> netSstoreResetRefund');
        }
    }
