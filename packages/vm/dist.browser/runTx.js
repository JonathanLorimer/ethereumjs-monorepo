...
async function _runTx(opts) {
    ...
    if (this._common.isActivatedEIP(3607) && !fromAccount.codeHash.equals(util_1.KECCAK256_NULL)) {
    ...
    if (this.DEBUG) {
        ...
        debug('-'.repeat(100));
        debug(`Received tx execResult: [ gasUsed=${gasUsed} exceptionError=${exceptionError ? `'${exceptionError.error}'` : 'none'} returnValue=0x${(0, util_1.short)(returnValue)} gasRefund=${results.gasRefund ?? 0} ]`);
    }
    /*
     * Parse results
     */
    ...
    if (this._common.isActivatedEIP(1153))
        this.evm._transientStorage.clear();
...
async function generateTxReceipt(tx, txResult, cumulativeGasUsed) {
    ...
    if (!tx.supports(tx_1.Capability.EIP2718TypedTransaction)) {
        // Legacy transaction
        if (this._common.gteHardfork(common_1.Hardfork.Byzantium)) {
            // Post-Byzantium
            receipt = {
